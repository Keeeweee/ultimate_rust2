...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!("The Frog hopped and has {} energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!("The Frog will go to sleep since it ran out of energy. Shhh!");
            self.sleep();
        }
    }
    pub fn sleep(&mut self) {
        if self.sleeping {
            ...
            error!("The Frod is already sleeping. Don't make any noises!")
        } else {
            self.sleeping = true;
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!("Default value for Frog was generated: {:?}", frog);
        frog
    }
}
